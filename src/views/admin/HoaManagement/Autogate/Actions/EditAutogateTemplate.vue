<template>
  <el-dialog
    ...
    center
  >
    <div v-if="templateLoading">Loading...</div>
    ...
    <form v-else>
      ...
      <div class="mb-4">
        ...
        <div class="mt-1 flex items-center">
          <img
            v-if="form.hoa_autogate_template_picture"
            :src="form.hoa_autogate_template_picture"
            :class="
...
watch(
  () => store.state.template.currentTemplate.data,
  (newVal, oldVal) => {
    form.value = { ...JSON.parse(JSON.stringify(newVal.data)) };
    form.value.hoa_autogate_template_second_page = content(
      newVal.data.hoa_autogate_template_second_page
    );
    form.value.hoa_autogate_template_footer_second_page = content(
      newVal.data.hoa_autogate_template_footer_second_page
    );
    background.value = newVal.data.background_image_id;
  }
);
...
async function handleSubmit() {
  ...
  const res = await store.dispatch("template/editTemplate", form.value);
  try {
